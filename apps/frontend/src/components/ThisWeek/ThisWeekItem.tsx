...
export const ThisWeekItem: React.FC<ThisWeekItemProps> = ({ item }) => {
  ...
  const handleSaveOrClose = async () => {
    ...
    if (editedTitle.trim() !== item.title) {
      try {
        await mutateItem(session, item._id, editedTitle.trim())
      } catch (error) {
  ...
  return (
    <motion.div
      ...
      key={item._id}
      ref={itemRef}
      className="flex flex-col text-left gap-1 p-4 hover:border border-border rounded-lg hover-bg group cursor-grab"
