...
export function useEvents() {
  ...
  const determineEventType = (title: string): "task" | "meeting" | "personal" => {
    const lowerTitle = title.toLowerCase()
    if (lowerTitle.includes("meet") || lowerTitle.includes("call") || lowerTitle.includes("sync")) {
      return "meeting"
    }
    if (lowerTitle.includes("task") || lowerTitle.includes("todo") || lowerTitle.includes("review")) {
      return "task"
    }
    return "personal"
  }
